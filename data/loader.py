...
class WineColorData(DataLoader):
    def __init__(self, path='data/winequality.csv', verbose=False, seed=1):
        super().__init__(path, verbose, seed)
    ...
    def data_name(self):
        return 'WineColorData'

    def class_column_name(self):
        return 'color'

    def _preprocess_data(self):
        to_encode = ["fixed acidity","volatile acidity","citric acid","residual sugar","chlorides","free sulfur dioxide","total sulfur dioxide","density","pH","sulphates","alcohol","quality","color"]
        # # label_encoder = preprocessing.LabelEncoder()
        ...
        df[to_encode] = df[to_encode].apply(pd.to_numeric, errors='floatdown')
